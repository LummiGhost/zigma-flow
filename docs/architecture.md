...
文档版本：v0.7（含 v0.2、v0.6、v0.7 修订增量，2026-07-17）
日期：2026-06-06
适用范围：Zigma Flow PRD v0.3 + v0.2/v0.6/v0.7 修订增量；v0.2 修订集中在 §5.2、§6.2、§7.1、§7.2；v0.7 修订集中在 §5.2、§6.2、§7.1、§7.2、§7.4

> **v0.7 修订总览（2026-07-17）：** 执行模型重构 — 以不可变记录（Attempt、Job Group Iteration）替代动态控制流。向前兼容：所有 v0.6 废弃特性在 v0.7 中内部翻译为新模型。
>
> **New modules（v0.7）：**
> - `src/engine/attemptModel.ts` — Attempt 生命周期纯函数：`deriveJobConclusion`、`retryPolicyAllowsRetry`、`classifyFailureKind`、`createOpenAttempt`、`sealAttempt`
> - `src/engine/jobGroupModel.ts` — Job Group 迭代状态机纯函数
> - `src/engine/outcomeModel.ts` — `computeJobConclusion(attempts, failurePolicy)`、`computeIterationConclusion(jobs)`、`computeRunConclusion(iterations)`
> - `src/expression/index.ts` — 新增 `invocation`、`attempt` 命名空间；状态函数 `success()`/`failure()`/`always()`/`cancelled()`（预解析，不改变语法）
> - `src/context/index.ts` — 新增 `buildExpressionContext()` 集中式辅助函数，替代 7 处内联构建
> - `src/events/eventTypes.ts` — 事件目录 45→55+：新增 3 个 attempt 生命周期事件、7 个 iteration/group 事件、更新 job_failed/job_blocked/job_retrying payload
>
> **Deprecation status update（v0.7）：**
>
> v0.6 废弃特性在 v0.7 中**内部翻译**为新执行模型，继续正常工作，v1.0 将移除：
>
> | v0.6 废弃特性 | v0.7 内部翻译 |
> |--------------|--------------|
> | `goto_step` / `goto_job` | 隐式 Job Group Iteration |
> | `retry_job` / `retry_with` | Attempt 模型（`retry.when` + `failure_kind`） |
> | `max_visits` | `repeat.max_iterations` |
> | `on_failure`（对象形式） | `failure_policy` 规范化 |
> | signals, `optional_needs`, `activation: manual`, `type: check` | 继续警告，v1.0 移除 |
>
...
| `engine` | 解释状态机，推进 step/job/run，处理 signal、router、retry、activation；**v0.2**：处理 status return（applyStatusReturn）、context patch（applyContextPatch）、step `if` 求值（evaluateStepCondition）、goto_step 跳转、max_visits 守门；**v0.7**：管理 Attempt 生命周期（createOpenAttempt/sealAttempt）、Job Group 迭代（advanceIteration）、concurrency group 过滤（applyConcurrencyPolicies）、failure policy 级联（evaluateFailurePolicy）、派生 job/iteration/run conclusion | 不直接调用 execa、simple-git 或 terminal 输出；**v0.2**：不允许 patch 触及状态机字段（jobs/signals/attempts/last_event_id 等）；**v0.7**：scheduler 保持纯函数 |
...
src/
  ...
  dag/
  engine/
    attemptModel.ts     # v0.7: Attempt 生命周期纯函数
    jobGroupModel.ts    # v0.7: Job Group 迭代状态机纯函数
    outcomeModel.ts     # v0.7: Outcome/Conclusion 计算纯函数
    scheduler.ts        # v0.7 增强：concurrency group 过滤
    routing.ts          # v0.7 增强：隐式 group 创建
    runAll.ts           # v0.7 增强：pre-scheduler mutation、failure policy cascade
    recordAgentFailure.ts  # v0.7 增强：attempt sealing、failure_kind 映射
  context/
  prompt/
  script/
  check/
...
- v0.7 新增的 `attemptModel`、`jobGroupModel`、`outcomeModel` 都是纯函数模块 — 输入状态，输出新状态或决策，不依赖任何外部适配器。
...
| Context | 核心概念 | 所有权 |
| --- | --- | --- |
| Workflow Definition | Workflow、JobDefinition、StepDefinition、SignalDefinition、PermissionPolicy、JobGroupDef、RepeatConfig、ConcurrencyConfig | workflow loader 和 schema validator |
| Skill Pack | SkillPack、SkillExport、SkillLock、AgentFunction、Policy | skill-pack loader 和 lock resolver |
| Run Runtime | Run、JobRun、StepRun、Attempt、JobGroupState、IterationState、StateTransition、GateDecision | workflow engine |
| Execution Records (v0.7) | Attempt（不可变记录，per-job 单调编号）、FailureKind（7 个已知值 + 扩展槽）、RetryPolicy（`when` 白名单）、AttemptOutcome（技术结果）、JobConclusion（业务结论） | engine/attemptModel、engine/outcomeModel |
| Job Group (v0.7) | JobGroupState、IterationState、RepeatConfig、`iteration.previous` 数据流 | engine/jobGroupModel |
| Concurrency (v0.7) | ConcurrencyGroup（静态 key + policy：allow/queue/cancel_previous/reject） | engine/scheduler（纯函数过滤）、engine/runAll（pre-scheduler mutation） |
...
- **v0.7**：`state.json` 新增 `job_groups` 段（`Record<string, JobGroupState>`）。Run 的 conclusion 由所有 group/iteration 的最严重结论派生（`computeRunConclusion`）。
...
- **v0.7**：每个 Job 有 `attempts: Attempt[]`（不可变记录数组）和 `attempt_count`（当前计数）。Job 的 `conclusion` 由所有 Attempt 和 `failure_policy` 派生（`deriveJobConclusion`）。Job 可选属于一个 Job Group（`group` 字段）。`step_visits` 和 `max_visits` 废弃，由 `repeat.max_iterations` 替代。
...
- Attempt 是 per-job 的不可变执行记录，单调编号（1, 2, 3…within each job）。
- 每个 Attempt 有独立的 `status`（`success` | `failure` | `cancelled`）和可选的 `failure_kind`。
- `failure_kind` 使用封闭字符串联合类型：`timeout` | `infrastructure_error` | `invalid_output` | `agent_error` | `cancelled` | `permission_denied` | `config_error` + `(string & {})` 扩展槽。
- Attempt 状态采用混合存储：关键汇总字段在 `state.json` 中，步骤详情在 events 中。不重复存储事件数据。
- 重试总是创建新 Attempt；历史 Attempt 永不被修改。
- `RetryPolicy` 定义何时重试：`when` 数组是 FailureKind 白名单，默认仅包含瞬时故障（`timeout`、`infrastructure_error`、`agent_error`）。配置/权限错误默认不重试。
- `JobConclusion` 由最后 Attempt 的 `AttemptOutcome` + `failure_policy` 派生（纯函数 `deriveJobConclusion`）。

JobGroup / Iteration（v0.7 新增聚合）：

- `JobGroupState` 收集同一 `group` 的所有 job，包含 `IterationState[]`。
- 迭代顺序执行：迭代 N 全部完成后才启动 N+1。迭代内 job 并行度由现有 scheduler 控制。
- `repeat.max_iterations` 是硬上界；`repeat.until` 在每个迭代结束后求值。
- `iteration.previous.jobs.<id>.outputs.<key>` 是唯一的跨迭代数据通道（深度限制放宽至 4）。
- 无 `group` 字段的 Job 执行恰好一次（单迭代，无 repeat）。
- 隐式 group：当 `goto_step`/`goto_job` 在未分组的 job 上触发时，Engine 创建隐式 Job Group（`__implicit__` 前缀），`max_visits` 翻译为 `max_iterations`。

ConcurrencyGroup（v0.7 新增聚合）：

- 静态字符串 key（`concurrency.group`）标识并发组。同组 job 共享并发槽。
- 四种 policy：`allow`（直接运行）、`queue`（等待槽释放）、`cancel_previous`（取消运行中 job）、`reject`（立即失败）。
- 三层执行：`queue`/`allow` 在 scheduler 纯函数过滤；`cancel_previous`/`reject` 在 pre-scheduler mutation 步骤。
- `cancel_previous` 仅取消同组运行中 job；已完成 job 不受影响（符合 forward-only 原则）。

FailurePolicy（v0.7 新增聚合）：

- Job 级 `failure_policy`：`fail`（默认，向上传播）、`continue`（标记 failed 但继续迭代）、`block`（立即阻塞）。
- 层级级联：job → iteration → group → run。各级可包含或升级故障。
- `continue` 产生 `success_with_warnings` JobConclusion（当迭代 otherwise 成功时）。不伪装成功。
...
v0.7 追加执行模型入口（#247–#250）：

```text
createOpenAttempt(runId, jobId)                    # 创建新 Attempt（status: success，open）
sealAttempt(runId, jobId, outcome, failureKind?)   # 封印 Attempt（不可变终态）
deriveJobConclusion(attempts, failurePolicy)        # 纯函数：Attempt[] + failure_policy → JobConclusion
classifyFailureKind(errorType)                      # 纯函数：errorType → FailureKind
retryPolicyAllowsRetry(policy, failureKind)         # 纯函数：RetryPolicy + FailureKind → boolean

advanceIteration(runId, groupId)                    # 完成当前迭代，求值 until，决定是否启动下一迭代
createImplicitGroup(runId, jobId)                   # 为未分组 job 创建隐式 Job Group（goto 向后兼容）

applyConcurrencyPolicies(state, workflow)           # Pre-scheduler mutation：执行 cancel_previous / reject
filterByConcurrencyGroup(batch, state, workflow)   # Scheduler 纯函数过滤：执行 queue / allow

evaluateFailurePolicy(job, state)                   # 求值 job 级 failure_policy → fail / continue / block
computeJobConclusion(attempts, failurePolicy)       # 纯函数：Attempt[] + failure_policy → JobConclusion
computeIterationConclusion(jobs)                    # 纯函数：Job[] → IterationConclusion
computeRunConclusion(iterations)                    # 纯函数：IterationConclusion[] → RunConclusion

buildExpressionContext(opts)                        # 集中式上下文构建（src/context/index.ts），替代 7 处内联
resolveStatusFunctions(expr, ctx, scope)            # 预解析状态函数（src/expression/index.ts），tokenization 前替换
```

所有新入口仍遵守 §7.1 既有约束：CLI 不直接改 state，所有写入走 StateStore + EventWriter；patch 类入口拒绝触及保留字段。v0.7 新增的 `attemptModel`、`jobGroupModel`、`outcomeModel` 都是纯函数模块 — 输入状态，输出新状态或决策。
...
Attempt 创建时 status 为 `success`（乐观），被 Engine 封印后不可变。每个 Job 的 Attempt 单调编号（1, 2, 3…）。重试创建新 Attempt，历史 Attempt 永不被修改。

【v0.7 修订】Job Group / Iteration 状态转换：
...
JobGroup:
  pending → running → completed     # 所有 iteration 完成
  pending → running → blocked       # 某 iteration blocked
  pending → running → failed        # 某 iteration failed 且 failure_policy: fail
```

迭代顺序执行。每个迭代内 job 并行度由 scheduler 控制（同 v0.2 并发模型）。`repeat.until` 在迭代完成后求值。

【v0.7 修订】Outcome vs Conclusion 模型：

- **AttemptOutcome**（技术层）：`success` | `failure` | `cancelled` — Engine 在 Attempt 封印时设置。
- **JobConclusion**（业务层）：`success` | `success_with_warnings` | `failure` | `blocked` | `cancelled` — 由最后 Attempt outcome + `failure_policy` 派生。
- **IterationConclusion**：由迭代内所有 Job 的 conclusion 派生（`computeIterationConclusion`）。
- **RunConclusion**：由所有 group/iteration 的 conclusion 派生（`computeRunConclusion`）。

固定规则：任一 Attempt success → JobConclusion `success`；全部 failure → 求值 `failure_policy`。`failure_policy: continue` + 失败的 job → `success_with_warnings`。
...
### 7.4 Concurrency model (v0.2, enhanced v0.7)
...
**Concurrency Group 层（v0.7 新增）：**

7. **Concurrency 是独立约束层**：在 parallelism cap（第 2 条）之后应用。两层是正交的——parallelism 控制总量，concurrency group 控制同组互斥。
8. **三层执行**：
   - **Pre-scheduler mutation**（`src/engine/runAll.ts`）：在 scheduler 选择 job 之前执行 `cancel_previous` 和 `reject` policy。这里涉及状态变更（取消运行中 job、标记 reject 的 job 为 failed）。
   - **Scheduler 纯函数过滤**（`src/engine/scheduler.ts`）：在 `selectExecutable` 内部执行 `queue` 和 `allow` policy。纯函数，不改变状态。
   - **Executor**：scheduler 返回的批次保证不违反 concurrency group 约束。
9. **`cancel_previous` 语义**：仅取消同 concurrency group 中正在运行的 job；已完成 job 不受影响。被取消 job 进入 `cancelled` 状态，`failure_kind = "cancelled"`。符合 forward-only 原则。
10. **Concurrency group key 是静态字符串**：由 `concurrency.group` 字段声明。表达式求值的动态 key 留待后续版本。
...
**Accepted Artifact Cost 优化（Issue #286 Phase 4）：** 策略位于 step 级 `constraints.routing_policy.objective`（`quality`=Phase 3 验收优先，缺省即此，行为与 Phase 3 逐字节一致；`accepted_artifact_cost`=Phase 4 AAC 排序）。metrics 记录无真实 token 费用，v1 采用文档化的确定性代理成本模型：每条记录贡献 `cost_class_weight(cost_class) × duration_ms` 代理成本单位（low=1/medium=2/high=4，缺省 high）；聚合器（`history.ts` 扩展）按 (task class, model) 累计 `execution_cost`（全部记录）、`retry_overhead`（attempt>1）、`rework_overhead`（report 被拒），`delivered_cost` 为三者之和，`AAC = delivered_cost / max(accepted, 1)`。AAC 策略下排序分层：有接受产物的按 AAC 升序（平局 acceptance 降序 → 声明序）；有样本但零接受（从未交付）按 delivered_cost 升序、排在有接受产物之后；零样本最后。硬约束过滤先于任何评分，评分绝不重新放行被过滤的 profile；显式 override 与 Phase 1/3 回退行为不变。`routing_reason` 携带 `aac-ranked ...` 依据；完整评分明细（aac/delivered_cost/execution_cost/retry_overhead/rework_overhead + 原始计数）经 run log 的 `writeSystemDetached` 保留——无新事件类型、无新 payload 字段（59 事件冻结）。`zigma-flow model-history` 命令复用同一聚合输出 per-task-class per-model 的验收与 AAC 报表（Phase 4 inspect/report 验收标准）。
