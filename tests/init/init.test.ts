...
describe("runInit integration", () => {
  ...
  // skill-lock.json is no longer generated by init (v0.6 deprecation)
  const expectedTopLevel = ["workflows", "skills", "runs", "config.json"];
  const expectedSkillFiles = [
    join("skills", "zigma-flow", "skill.yml"),
    join("skills", "zigma-flow", "SKILL.md"),
  ...
  it("does NOT generate skill-lock.json (deprecated in v0.6) (T-INIT-8)", async () => {
    ...
    // skill-lock.json should NOT exist — it is deprecated in v0.6
    const lockPath = join(tempDir, ".zigma-flow", "skill-lock.json");
  ...
  it("generates the Zigma Flow usage skill without overwriting customization (T-INIT-10A)", async () => {
    const { error } = await safeRunInit(tempDir);
    expect(error).toBeUndefined();

    const skillPath = join(tempDir, ".zigma-flow", "skills", "zigma-flow", "SKILL.md");
    const packRoot = join(tempDir, ".zigma-flow", "skills", "zigma-flow");
    const skillPack = await loadSkillPack(packRoot);
    const generated = await readFile(skillPath, "utf-8");
    ...
    expect(generated).toContain("Engine owns state transitions");
    expect(generated).not.toContain("agents/openai.yaml");
    ...
    expect(second.error).toBeUndefined();
    expect(await readFile(skillPath, "utf-8")).toBe(customized);
    expect(second.result?.files).toContainEqual({ path: skillPath, status: "skipped" });
  });
...
describe("code-change template (WF-P10-WORKFLOW)", () => {
  ...
  it("runInit writes the full P10 file set (TC-WORKFLOW-1)", async () => {
    ...
    // skill-lock.json is no longer generated (v0.6 deprecation)
    const expected = [
      "config.json",
...
describe("v0.4 init environment detection (WF-INIT)", () => {
  ...
  // T-INIT-28
  it("skill-lock.json is not generated; skill pack loads from direct path (T-INIT-28)", async () => {
    await setupProject("pnpm", { typecheck: "tsc", lint: "eslint .", "test:ci": "vitest run" });
    const { error } = await safeRunInit(tempDir);
    expect(error).toBeUndefined();

    // skill-lock.json should NOT exist — deprecated in v0.6
    const lockPath = join(dotZigma, "skill-lock.json");
    expect(await pathExists(lockPath)).toBe(false);

    // Skill pack must still load directly from the generated directory.
    const packRoot = join(dotZigma, "skills", "code-change");
    const def = await loadSkillPack(packRoot);
    expect(def.id).toBe("zigma.code-change");
  });
